...
void Hexapod::msgShow(void)
{
        ...
        // std::cout<<leg_root.foot_des_pos*100<<std::endl;
        // std::cout<<"leg_root.foot_act_pos"<<std::endl;
        // std::cout<<leg_root.foot_act_pos*100<<std::endl;

        // std::cout<<"leg_root.joint_des_pos"<<std::endl;
        // std::cout<<leg_root.joint_des_pos<<std::endl;
        // std::cout<<"leg_root.joint_rec_pos" <<std::endl;
        // std::cout<<leg_root.joint_rec_pos <<std::endl;
        ...
        // std::cout<<" body_root.imu_ang_vel"<<std::endl;
        // std::cout<< body_root.imu_ang_vel.transpose() <<std::endl;

        // std::cout<<" body_root.imu_lin_acc"<<std::endl;
        // std::cout<< body_root.imu_lin_acc.transpose() <<std::endl;

        // std::cout<<" body_root.eulerAngle"<<std::endl;
        // std::cout<< body_root.eulerAngle.transpose() <<std::endl;
        // printf("\n -------next-----\n");
}
...
{       
		...
		HarBri.dataIntegrat();
                ...
                // //xzb230522
                hard_ware_imu_mtx.lock();
                body_root.imu_ang_vel<< _vectorNavData.gyr[0], _vectorNavData.gyr[1], _vectorNavData.gyr[2];
                body_root.imu_lin_acc<< _vectorNavData.acc[0], _vectorNavData.acc[1], _vectorNavData.acc[2];
                body_root.imu_eulerAngle << _vectorNavData.euler[0], _vectorNavData.euler[1], _vectorNavData.euler[2];
                body_root.eulerAngle = body_root.imu_eulerAngle;
                hard_ware_imu_mtx.unlock();
        ...
        */
                ...
                leg_root.joint_rec_pos.block<1,9>(0,9)=-leg_root.joint_rec_pos.block<1,9>(0,9);
                leg_root.joint_rec_vel.block<1,9>(0,9)=-leg_root.joint_rec_vel.block<1,9>(0,9);
                leg_root.joint_rec_tor.block<1,9>(0,9)=-leg_root.joint_rec_tor.block<1,9>(0,9);
                ...
                // leg_root.joint_rec_vel(9)=-leg_root.joint_rec_vel(9);leg_root.joint_rec_vel(10)=-leg_root.joint_rec_vel(10);leg_root.joint_rec_vel(11)=-leg_root.joint_rec_vel(11);
                // leg_root.joint_rec_vel(12)=-leg_root.joint_rec_vel(12);leg_root.joint_rec_vel(13)=-leg_root.joint_rec_vel(13);leg_root.joint_rec_vel(14)=-leg_root.joint_rec_vel(14);
                // leg_root.joint_rec_vel(15)=-leg_root.joint_rec_vel(15);leg_root.joint_rec_vel(16)=-leg_root.joint_rec_vel(16);leg_root.joint_rec_vel(17)=-leg_root.joint_rec_vel(17);
        
                // leg_root.joint_rec_tor(9)=-leg_root.joint_rec_tor(9);leg_root.joint_rec_tor(10)=-leg_root.joint_rec_tor(10);leg_root.joint_rec_tor(11)=-leg_root.joint_rec_tor(11);
                // leg_root.joint_rec_tor(12)=-leg_root.joint_rec_tor(12);leg_root.joint_rec_tor(13)=-leg_root.joint_rec_tor(13);leg_root.joint_rec_tor(14)=-leg_root.joint_rec_tor(14);
                // leg_root.joint_rec_tor(15)=-leg_root.joint_rec_tor(15);leg_root.joint_rec_tor(16)=-leg_root.joint_rec_tor(16);leg_root.joint_rec_tor(17)=-leg_root.joint_rec_tor(17);
        #endif
...
void Hexapod::recDataHandling(void) 
{
        ...
        #if HARD_WARE==1
                leg_root.foot_act_pos.block<3,1>(0,5)=_kinematic[5].forwardKinematic(5, leg_root.joint_rec_pos.block<1,3>(0,15).transpose()-HarBri.rb_off_set);
                leg_root.foot_act_pos.block<3,1>(0,4)=_kinematic[4].forwardKinematic(4, leg_root.joint_rec_pos.block<1,3>(0,12).transpose()-HarBri.rm_off_set);
                leg_root.foot_act_pos.block<3,1>(0,3)=_kinematic[3].forwardKinematic(3, leg_root.joint_rec_pos.block<1,3>(0,9).transpose()-HarBri.rf_off_set);
                leg_root.foot_act_pos.block<3,1>(0,2)=_kinematic[2].forwardKinematic(2, -leg_root.joint_rec_pos.block<1,3>(0,6).transpose()+HarBri.lb_off_set);
                leg_root.foot_act_pos.block<3,1>(0,1)=_kinematic[1].forwardKinematic(1, -leg_root.joint_rec_pos.block<1,3>(0,3).transpose()+HarBri.lm_off_set);
                leg_root.foot_act_pos.block<3,1>(0,0)=_kinematic[0].forwardKinematic(0, -leg_root.joint_rec_pos.block<1,3>(0,0).transpose()+HarBri.lf_off_set);
        #elif HARD_WARE==2
                leg_root.foot_act_pos.block<3,1>(0,5)=_kinematic[5].forwardKinematic(5, leg_root.joint_rec_pos.block<1,3>(0,15).transpose());
                leg_root.foot_act_pos.block<3,1>(0,4)=_kinematic[4].forwardKinematic(4, leg_root.joint_rec_pos.block<1,3>(0,12).transpose());
                leg_root.foot_act_pos.block<3,1>(0,3)=_kinematic[3].forwardKinematic(3, leg_root.joint_rec_pos.block<1,3>(0,9).transpose());
                leg_root.foot_act_pos.block<3,1>(0,2)=_kinematic[2].forwardKinematic(2, leg_root.joint_rec_pos.block<1,3>(0,6).transpose());
                leg_root.foot_act_pos.block<3,1>(0,1)=_kinematic[1].forwardKinematic(1, leg_root.joint_rec_pos.block<1,3>(0,3).transpose());
                leg_root.foot_act_pos.block<3,1>(0,0)=_kinematic[0].forwardKinematic(0, leg_root.joint_rec_pos.block<1,3>(0,0).transpose());
        #endif
        ...
        #if HARD_WARE==1
                leg_root.foot_des_pos.block<3,1>(0,5)=_kinematic[5].forwardKinematic(5, leg_root.joint_des_pos.block<3,1>(0,5)-HarBri.rb_off_set);
                leg_root.foot_des_pos.block<3,1>(0,4)=_kinematic[4].forwardKinematic(4, leg_root.joint_des_pos.block<3,1>(0,4)-HarBri.rm_off_set);
                leg_root.foot_des_pos.block<3,1>(0,3)=_kinematic[3].forwardKinematic(3, leg_root.joint_des_pos.block<3,1>(0,3)-HarBri.rf_off_set);
                leg_root.foot_des_pos.block<3,1>(0,2)=_kinematic[2].forwardKinematic(2, -leg_root.joint_des_pos.block<3,1>(0,2)+HarBri.lb_off_set);
                leg_root.foot_des_pos.block<3,1>(0,1)=_kinematic[1].forwardKinematic(1, -leg_root.joint_des_pos.block<3,1>(0,1)+HarBri.lm_off_set);
                leg_root.foot_des_pos.block<3,1>(0,0)=_kinematic[0].forwardKinematic(0, -leg_root.joint_des_pos.block<3,1>(0,0)+HarBri.lf_off_set);
        #else HARD_WARE==2
                leg_root.foot_des_pos.block<3,1>(0,5)=_kinematic[5].forwardKinematic(5, leg_root.joint_des_pos.block<3,1>(0,5));
                leg_root.foot_des_pos.block<3,1>(0,4)=_kinematic[4].forwardKinematic(4, leg_root.joint_des_pos.block<3,1>(0,4));
                leg_root.foot_des_pos.block<3,1>(0,3)=_kinematic[3].forwardKinematic(3, leg_root.joint_des_pos.block<3,1>(0,3));
                leg_root.foot_des_pos.block<3,1>(0,2)=_kinematic[2].forwardKinematic(2, leg_root.joint_des_pos.block<3,1>(0,2));
                leg_root.foot_des_pos.block<3,1>(0,1)=_kinematic[1].forwardKinematic(1, leg_root.joint_des_pos.block<3,1>(0,1));
                leg_root.foot_des_pos.block<3,1>(0,0)=_kinematic[0].forwardKinematic(0, leg_root.joint_des_pos.block<3,1>(0,0));
        #endif
        ...
        // lcc 20230513: 足端力(计算公式：F= (j_T)inv * tor );jacobiTranInv()中还计算了雅克比矩阵  
                ...
                leg_root.foot_act_force.block<3,1>(0,5)=_kinematic[5].jacobiTranInv(5, leg_root.joint_rec_pos.block<1,3>(0,15).transpose()-HarBri.rb_off_set) * leg_root.joint_rec_tor.block<1,3>(0,15).transpose();
                leg_root.foot_act_force.block<3,1>(0,4)=_kinematic[4].jacobiTranInv(4, leg_root.joint_rec_pos.block<1,3>(0,12).transpose()-HarBri.rm_off_set) * leg_root.joint_rec_tor.block<1,3>(0,12).transpose();
                leg_root.foot_act_force.block<3,1>(0,3)=_kinematic[3].jacobiTranInv(3, leg_root.joint_rec_pos.block<1,3>(0,9).transpose()-HarBri.rf_off_set) * leg_root.joint_rec_tor.block<1,3>(0,9).transpose();
                leg_root.foot_act_force.block<3,1>(0,2)=_kinematic[2].jacobiTranInv(2, -leg_root.joint_rec_pos.block<1,3>(0,6).transpose()+HarBri.lb_off_set) * -leg_root.joint_rec_tor.block<1,3>(0,6).transpose();
                leg_root.foot_act_force.block<3,1>(0,1)=_kinematic[1].jacobiTranInv(1, -leg_root.joint_rec_pos.block<1,3>(0,3).transpose()+HarBri.lm_off_set) * -leg_root.joint_rec_tor.block<1,3>(0,3).transpose();
                ...
                leg_root.foot_act_force.block<3,1>(0,1)=_kinematic[1].jacobiTranInv(1,leg_root.joint_rec_pos.block<1,3>(0,3).transpose()) * leg_root.joint_rec_tor.block<1,3>(0,3).transpose();
                leg_root.foot_act_force.block<3,1>(0,2)=_kinematic[2].jacobiTranInv(2,leg_root.joint_rec_pos.block<1,3>(0,6).transpose()) * leg_root.joint_rec_tor.block<1,3>(0,6).transpose();
                leg_root.foot_act_force.block<3,1>(0,3)=_kinematic[3].jacobiTranInv(3,leg_root.joint_rec_pos.block<1,3>(0,9).transpose()) * leg_root.joint_rec_tor.block<1,3>(0,9).transpose();
                leg_root.foot_act_force.block<3,1>(0,4)=_kinematic[4].jacobiTranInv(4,leg_root.joint_rec_pos.block<1,3>(0,12).transpose()) * leg_root.joint_rec_tor.block<1,3>(0,12).transpose();
                leg_root.foot_act_force.block<3,1>(0,5)=_kinematic[5].jacobiTranInv(5,leg_root.joint_rec_pos.block<1,3>(0,15).transpose()) * leg_root.joint_rec_tor.block<1,3>(0,15).transpose();
        #endif
...
void Hexapod::realHexMsgLoadAndSend(void)
{
        ...
        mtx.lock();
                ...
                // kp_temp=leg_root.joint_kp;
                // kd_temp=leg_root.joint_kd;
                ...
                HarBri.lf_des_kp=leg_root.joint_kp.block<3,1>(0,0);
                HarBri.lm_des_kp=leg_root.joint_kp.block<3,1>(0,1);
                HarBri.lb_des_kp=leg_root.joint_kp.block<3,1>(0,2);
                HarBri.rf_des_kp=leg_root.joint_kp.block<3,1>(0,3);
                HarBri.rm_des_kp=leg_root.joint_kp.block<3,1>(0,4);
                HarBri.rb_des_kp=leg_root.joint_kp.block<3,1>(0,5);

                HarBri.lf_des_kd=leg_root.joint_kd.block<3,1>(0,0);
                HarBri.lm_des_kd=leg_root.joint_kd.block<3,1>(0,1);
                HarBri.lb_des_kd=leg_root.joint_kd.block<3,1>(0,2);
                HarBri.rf_des_kd=leg_root.joint_kd.block<3,1>(0,3);
                HarBri.rm_des_kd=leg_root.joint_kd.block<3,1>(0,4);
                HarBri.rb_des_kd=leg_root.joint_kd.block<3,1>(0,5);
