...
{
        step_length=_RbTraj.retDesStepLength();  //步长
        des_vel=step_length/dt_s;    //期望速度＝步长/时间　　　
        // std::cout<<"step_length: "<<step_length<<std::endl;
        // std::cout<<"des_vel: "<<des_vel<<std::endl;
        // std::cout<<"retDesStepHight: "<<_RbTraj.retDesStepHight()<<std::endl;

        //机器坐标系方向跟据右手定则： 向前为x正，向左为y正，向上为z正。
        //lcc 20230513: 跟关节下真实足端位置
        leg_root.foot_act_pos.block<3,1>(0,5)=_RbKin.forwardKinematic(leg_root.joint_rec_pos.block<1,3>(0,15).transpose());
        leg_root.foot_act_pos.block<3,1>(0,4)=_RmKin.forwardKinematic(leg_root.joint_rec_pos.block<1,3>(0,12).transpose());
        leg_root.foot_act_pos.block<3,1>(0,3)=_RfKin.forwardKinematic(leg_root.joint_rec_pos.block<1,3>(0,9).transpose());
        leg_root.foot_act_pos.block<3,1>(0,2)=_LbKin.forwardKinematic(leg_root.joint_rec_pos.block<1,3>(0,6).transpose());
        leg_root.foot_act_pos.block<3,1>(0,1)=_LmKin.forwardKinematic(leg_root.joint_rec_pos.block<1,3>(0,3).transpose());
        leg_root.foot_act_pos.block<3,1>(0,0)=_LfKin.forwardKinematic(leg_root.joint_rec_pos.block<1,3>(0,0).transpose());
        ...
        body_root.leg_origin.block<3,1>(0,5)=_Rb._FooBodAdjMap.legOriRefToBody();
        body_root.leg_origin.block<3,1>(0,4)=_Rm._FooBodAdjMap.legOriRefToBody();
        body_root.leg_origin.block<3,1>(0,3)=_Rf._FooBodAdjMap.legOriRefToBody();
        body_root.leg_origin.block<3,1>(0,2)=_Lb._FooBodAdjMap.legOriRefToBody();
        body_root.leg_origin.block<3,1>(0,1)=_Lm._FooBodAdjMap.legOriRefToBody();
        body_root.leg_origin.block<3,1>(0,0)=_Lf._FooBodAdjMap.legOriRefToBody();
        ...
        leg_root.foot_des_pos.block<3,1>(0,5)=_RbKin.forwardKinematic(leg_root.joint_des_pos.block<3,1>(0,5));
        leg_root.foot_des_pos.block<3,1>(0,4)=_RmKin.forwardKinematic(leg_root.joint_des_pos.block<3,1>(0,4));
        leg_root.foot_des_pos.block<3,1>(0,3)=_RfKin.forwardKinematic(leg_root.joint_des_pos.block<3,1>(0,3));
        leg_root.foot_des_pos.block<3,1>(0,2)=_LbKin.forwardKinematic(leg_root.joint_des_pos.block<3,1>(0,2));
        leg_root.foot_des_pos.block<3,1>(0,1)=_LmKin.forwardKinematic(leg_root.joint_des_pos.block<3,1>(0,1));
        leg_root.foot_des_pos.block<3,1>(0,0)=_LfKin.forwardKinematic(leg_root.joint_des_pos.block<3,1>(0,0));
        ...
        //               20230504:ok
        leg_root.foot_act_force.block<3,1>(0,0)=_LfKin.jacobiTranInv(1,leg_root.joint_rec_pos.block<1,3>(0,0).transpose())*leg_root.joint_rec_tor.block<1,3>(0,0).transpose();
        leg_root.foot_act_force.block<3,1>(0,1)=_LmKin.jacobiTranInv(2,leg_root.joint_rec_pos.block<1,3>(0,1).transpose())*leg_root.joint_rec_tor.block<1,3>(0,3).transpose();
        leg_root.foot_act_force.block<3,1>(0,2)=_LbKin.jacobiTranInv(3,leg_root.joint_rec_pos.block<1,3>(0,2).transpose())*leg_root.joint_rec_tor.block<1,3>(0,6).transpose();
        leg_root.foot_act_force.block<3,1>(0,3)=_RfKin.jacobiTranInv(4,leg_root.joint_rec_pos.block<1,3>(0,3).transpose())*leg_root.joint_rec_tor.block<1,3>(0,9).transpose();
        leg_root.foot_act_force.block<3,1>(0,4)=_RmKin.jacobiTranInv(5,leg_root.joint_rec_pos.block<1,3>(0,4).transpose())*leg_root.joint_rec_tor.block<1,3>(0,12).transpose();
        leg_root.foot_act_force.block<3,1>(0,5)=_RbKin.jacobiTranInv(6,leg_root.joint_rec_pos.block<1,3>(0,5).transpose())*leg_root.joint_rec_tor.block<1,3>(0,15).transpose();

        // lcc 20230513: 足端速度(计算公式：x_dot=j*q_dot);  
        //               修改雅克比矩阵可以修改速度值正负号来校正方向;
        //               仿真中已经验证过，是正确的;
        body_root.foot_act_vel.block<3,1>(0,0)=_LfKin.jacobi()*leg_root.joint_rec_vel.block<1,3>(0,0).transpose();
        body_root.foot_act_vel.block<3,1>(0,1)=_LmKin.jacobi()*leg_root.joint_rec_vel.block<1,3>(0,3).transpose();
        body_root.foot_act_vel.block<3,1>(0,2)=_LbKin.jacobi()*leg_root.joint_rec_vel.block<1,3>(0,6).transpose();
        body_root.foot_act_vel.block<3,1>(0,3)=_RfKin.jacobi()*leg_root.joint_rec_vel.block<1,3>(0,9).transpose();
        body_root.foot_act_vel.block<3,1>(0,4)=_RmKin.jacobi()*leg_root.joint_rec_vel.block<1,3>(0,12).transpose();
        body_root.foot_act_vel.block<3,1>(0,5)=_RbKin.jacobi()*leg_root.joint_rec_vel.block<1,3>(0,15).transpose();
}
...
void Hexapod::simMsgPub(void)
{
                ...
                if(joint_pos_run_count>=3) // lcc 20230405: 表示已经从机器人起始点角度向目标角度以等差逼近的方式，逼近了两步
                        {
                                #if SIM_PROTECT==1
                                        _LfDatUnuProtect.velLimAndDifFroDesPosAndActPos(1, 3,
                                        ...
                                        _LmDatUnuProtect.velLimAndDifFroDesPosAndActPos(2, 3,
                                        ...
                                        _LbDatUnuProtect.velLimAndDifFroDesPosAndActPos(3, 3,
                                        ...
                                        _RfDatUnuProtect.velLimAndDifFroDesPosAndActPos(4, 3,
                                        ...
                                        _RmDatUnuProtect.velLimAndDifFroDesPosAndActPos(5, 3,
                                        ...
                                        _RbDatUnuProtect.velLimAndDifFroDesPosAndActPos(6, 3,
                                ...
                                #endif
                                if(_RbDatUnuProtect.diff_val_flag==false or _RmDatUnuProtect.diff_val_flag==false 
                                or _RfDatUnuProtect.diff_val_flag==false or _LbDatUnuProtect.diff_val_flag==false 
                                or _LmDatUnuProtect.diff_val_flag==false or _LfDatUnuProtect.diff_val_flag==false)
                                {
                                        _RbDatUnuProtect.diff_val_flag=false;_RmDatUnuProtect.diff_val_flag=false;
                                        _RfDatUnuProtect.diff_val_flag=false;_LbDatUnuProtect.diff_val_flag=false;
                                        _LmDatUnuProtect.diff_val_flag=false;_LfDatUnuProtect.diff_val_flag=false;
                                        printf("\n   ------------diff_val_flag:%d --------------\n",_RbDatUnuProtect.diff_val_flag);
                                }
                                else if(_RbDatUnuProtect.vel_lim_flag==false or _RmDatUnuProtect.vel_lim_flag==false 
                                or _RfDatUnuProtect.vel_lim_flag==false or _LbDatUnuProtect.vel_lim_flag==false 
                                or _LmDatUnuProtect.vel_lim_flag==false or _LfDatUnuProtect.vel_lim_flag==false)
                                {
                                        _RbDatUnuProtect.vel_lim_flag=false;_RmDatUnuProtect.vel_lim_flag=false;
                                        _RfDatUnuProtect.vel_lim_flag=false;_LbDatUnuProtect.vel_lim_flag=false;
                                        _LmDatUnuProtect.vel_lim_flag=false;_LfDatUnuProtect.vel_lim_flag=false;
                                        printf("\n   ------------vel_lim_flag:%d --------------\n",_RbDatUnuProtect.vel_lim_flag);
                                }
                                else
                                {
        ...
        if(joint_pos_run_count>=3) // lcc 20230405: 表示已经从机器人起始点角度向目标角度以等差逼近的方式，逼近了两步
                {
                                ...
                                if (_RbDatUnuProtect.diff_val_flag == false or _RmDatUnuProtect.diff_val_flag == false or _RfDatUnuProtect.diff_val_flag == false or _LbDatUnuProtect.diff_val_flag == false or _LmDatUnuProtect.diff_val_flag == false or _LfDatUnuProtect.diff_val_flag == false)
                                {
                                        _RbDatUnuProtect.diff_val_flag = false;
                                        _RmDatUnuProtect.diff_val_flag = false;
                                        _RfDatUnuProtect.diff_val_flag = false;
                                        _LbDatUnuProtect.diff_val_flag = false;
                                        _LmDatUnuProtect.diff_val_flag = false;
                                        _LfDatUnuProtect.diff_val_flag = false;
                                        // printf("\n   ------------diff_val_flag:%d --------------\n",_RbDatUnuProtect.diff_val_flag);
                        }
                        else if(_RbDatUnuProtect.vel_lim_flag==false or _RmDatUnuProtect.vel_lim_flag==false 
                        or _RfDatUnuProtect.vel_lim_flag==false or _LbDatUnuProtect.vel_lim_flag==false 
