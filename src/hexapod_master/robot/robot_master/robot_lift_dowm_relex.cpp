...
Eigen::Matrix<double,3,6> rec_foot_pos;
Eigen::Matrix<double,1,6> rec_foot_pos_flag;
Eigen::Matrix<double,1,6> reset_rec_foot_pos_flag;
...
void Hexapod::liftFollowReaction(void)
{
    ...
    {   
        ...
        if( fabs( lift_x_traj_rec(i)- temp_x(0) ) >=0.75 *0.01 ) //如果在支撑态下，轨迹前进了　0.75 厘米, 那么复原 lamdaX lamdaY
        { 
            if( cpg_touch_down_scheduler(i)==0 && swing_traj_switch_flag[i]==0) //如果cpg是触地态
            {
                int tt=int(1/set_cpg_ctrl_cycle)*0.5*0.25;
        ...
        {  
            ...
            {   
                ...
                rec_foot_pos_flag(i)=1;
            }
        }
        // printf("i: %d\n",i);
        std::cout<<" cpg_touch_down_scheduler "<<std::endl;
        std::cout<< cpg_touch_down_scheduler <<std::endl;
        std::cout<<" rec_foot_pos_flag "<<std::endl;
        std::cout<< rec_foot_pos_flag <<std::endl;

        // 如果进入了轨迹抬升阶段，　且如果当前是支撑态，并且　腿抬高轨迹 foot_cross_traj-rec_foot_cross_traj　超过了xxcm
        if( rec_foot_pos_flag(i)==1 && cpg_touch_down_scheduler(i)==0 && 
            fabs( fabs( leg_root.foot_cross_traj(2,i) ) - fabs( rec_foot_cross_traj(2,i) ) ) >1*0.01) 
        {


            // rec_foot_pos.block<3,1>(0,i)=world_root.foot_des_pos.block<3,1>(0,i);
            rec_foot_pos(0,i)=world_root.foot_des_pos(0,i)-leg_root.step_set_length(i)*0.01; // 减去步长，相当于记录开始进入摆动态时的位置
            rec_foot_pos(1,i)=world_root.foot_des_pos(1,i);
            rec_foot_pos(2,i)=world_root.foot_des_pos(2,i);

            reset_rec_foot_pos_flag(i)=1;
            rec_foot_cross_traj=leg_root.foot_cross_traj;

            // std::cout<<"world_root.foot_des_pos"<<std::endl;
            // std::cout<<world_root.foot_des_pos<<std::endl;
            // std::cout<<"rec_foot_pos"<<std::endl;
            // std::cout<<rec_foot_pos<<std::endl;

            // printf("%d aaa\n",i);
            // exit(0);
        }
        else if(cpg_touch_down_scheduler(i)==1 && reset_rec_foot_pos_flag(i)==1)
        {
            rec_foot_pos_flag(i)==0;
            // printf("%d bbb\n",i);
            // exit(0);
        }
    }

    //记录前两条腿的抬升高度( 其实就是估计的障碍物高度 )，用来让后面的腿学习这个高度
    // printf("leg_root.foot_cross_object_est:%f  %f  \n",leg_root.foot_cross_object_est(0),leg_root.foot_cross_object_est(3));
    ...
    //记录前两条腿的抬升高度( 其实就是估计的障碍物高度 )，用来让后面的腿学习这个高度
    // rec_foot_pos.block<3,1>(0,0)=world_root.foot_des_pos.block<3,1>(0,0);
    // rec_foot_pos.block<3,1>(0,3)=world_root.foot_des_pos.block<3,1>(0,3);
    std::cout<<"world_root.foot_des_pos"<<std::endl;
    std::cout<<world_root.foot_des_pos<<std::endl;
    std::cout<<"rec_foot_pos"<<std::endl;
    std::cout<<rec_foot_pos<<std::endl;
