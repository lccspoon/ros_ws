...
void Hexapod::setStandPose(void)
{
        ...
        // 设置时，一定要是等差的
        int bb=0;
        static_pos<< (6.5+bb)*0.01, 9*0.01, -13.5*0.01;
        leg_root.foot_set_static_pos.block<3,1>(0,0)=static_pos;

        static_pos<< (4+bb)*0.01, 9*0.01, -13.5*0.01;
        leg_root.foot_set_static_pos.block<3,1>(0,1)=static_pos;

        static_pos<< (1.5+bb)*0.01, 9*0.01, -13.5*0.01;
            ...
            switch (key_value)
            {
                    ...
                    case 'h':
                            {  
                                ...
                                one.setOnes();
                                leg_root.step_set_hight=leg_root.step_set_hight+one*0.25*0.01;
                                for(int i=0; i<6; i++)
                                {
                                    if( leg_root.step_set_hight(i)<=0 )
                                        leg_root.step_set_hight(i)=0;

                                    if( leg_root.step_set_hight(i)>=9*0.01 )
                                        leg_root.step_set_hight(i)=9*0.01;
                    ...
                    case 'n':
                            {
                                Eigen::Matrix<double,1,6> one;
                                one.setOnes();
                                leg_root.step_set_length=leg_root.step_set_length+one*0.25*0.01;
                                for(int i=0; i<6; i++)
                                {
                                    if( leg_root.step_set_length(i)<=0 )
                                        leg_root.step_set_length(i)=0;

                                    if( leg_root.step_set_length(i)>=9*0.01 )
                                        leg_root.step_set_length(i)=9*0.01;
