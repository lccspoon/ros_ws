...
int Xsens_init()
{
		...
		else if (device->deviceId().isVru() || device->deviceId().isAhrs())
		{
			...
			// xzb230522  配置 
			configArray.push_back(XsOutputConfiguration(XDI_AccelerationHR, 400));//XDI_AccelerationHR
			configArray.push_back(XsOutputConfiguration(XDI_RateOfTurnHR, 400));//XDI_RateOfTurnHR
		}
